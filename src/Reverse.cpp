#ifndef __REVERSE_CPP__
#define __REVERSE_CPP__

#include "../header/Reverse.hpp"


Reverse::Reverse(Cmd* c1, Cmd* c2, bool first, bool last) {
    this->l1 = c1->getArgs();
    this->l2 = c2->getArgs();
    this->left = c1;
    this->right = c2;
    ...
    if(l2.size() > 0) fd = open(l2[0].c_str(), O_RDONLY);
    else {
...
int Reverse::getStatus() { return this->status; }

pid_t Reverse::getPid() { return this->pid; }

string Reverse::getLabel() {
    if(l2.size() > 0) return l2[0];
}

vector<string> Reverse::getArgs() {
